...
public class GitHubPRSkipCommentEvent extends GitHubPREvent {
     ...
     */
    @Override
    public boolean isSkip(GitHubPRTrigger gitHubPRTrigger, GHPullRequest remotePR, GitHubPRPullRequest localPR) {
        boolean ret = false;
        ...
        if (StringUtils.isNotBlank(pullRequestBody)) {
            ...
            for (String skipBuildPhrase : skipBuildPhrases) {
                ...
                if (skipBuildPhrasePattern.matcher(pullRequestBody).matches()) {
                    LOGGER.log(Level.INFO, "Pull request commented with {0} skipBuildPhrase. Hence skipping the buildAndComment.", skipBuildPhrase);
                    ret = true;
                    break;
                }
            }
